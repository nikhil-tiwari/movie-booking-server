...
const handleGetTheatre = async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({ status: "Theatre Fetched successfully", data: allTheatres, page: page });
    } catch (err) {
...
const handleAddMovieForListing = async (req, res) => {
    ...
    const movie = await Movie.findById(movieToAdd);
    if(!movie) {
    ...
    try {
        ...
        const isMovieAlreadyListed = theatreToUpdate.movieListed.some(
            (listedMovieId) => listedMovieId.toString() === movieToAdd.toString()
        );

        if (isMovieAlreadyListed) {
            return res.status(400).json({ status: "Failed", message: "Movie is already listed in this theatre" });
        }
        theatreToUpdate.movieListed.push(movieToAdd);
        ...
        if(!theatreToUpdate) {
            return res.status(404).json({ status: "Failed", message: "Theatre does not exist" });
        }
        return res.status(500).json({ status: "Internal server error", error: err });
    }
}
